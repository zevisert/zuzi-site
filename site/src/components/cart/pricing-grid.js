    ...
    return html`
      ...
      <style>
        ...
        .row {
          ...
          grid-template-areas:
            "decrement increment medium price"
            ...
            ${(() => {
              if (pricing.available) {
                return html`
                  ...
                  <mwc-icon
                    class="increment"
                    @click=${() => this._incrementItem(pricing)}
                    >add_circle_outline</mwc-icon
                  >
                  <mwc-icon
                    class="decrement"
                    @click=${() => this._decrementItem(pricing)}
                    >remove_circle_outline</mwc-icon
                  >
                `;
              } else {
                return html` <div class="pricing">Sold out!</div> `;
              }
            })()}
            <div class="sizing">
              ${pricing.size.width}x${pricing.size.height} ${pricing.size.unit}
            </div>
          </div>`
        )}
      </div>
      <div class="summary">
        <div class="tally">
          <div class="total">
            Total:
            $${[...this.basket.entries()].reduce(
              (total, [pricing, count]) => total + pricing.price * count,
              0
            )}
          </div>
          <button
            ...
            ?disabled="${[...this.basket.values()].every((val) => val === 0)}"
          >
            Add to cart
          </button>
        </div>
